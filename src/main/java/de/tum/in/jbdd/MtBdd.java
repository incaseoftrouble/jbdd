...
public interface MtBdd extends BooleanDecisionDiagram {
    ...
    /**
     * Iterates over all assignments under which the given {@code function} evaluates to one of the given
     * {@code values}, in lexicographic ascending order. {@code values} may be {@code null}, meaning "any
     * value" - then this iterates every assignment. {@link ValuedIterator#value()} yields the value the
     * function takes under the assignment just returned.
     *
     * <p><b>Note:</b> The returned iterator is lazy and consults diagram-internal state that the next
     * query overwrites. It must be fully drained before any other operation is invoked on this diagram;
     * interleaving is checked by an assertion, not supported.</p>
     *
     * <p><b>Note:</b> The returned bit set is modified in-place. If all assignments should be gathered
     * into a set or similar, they have to be cloned after each call to {@link Iterator#next()}.</p>
     */
    ValuedIterator<BitSet> assignmentIterator(int function, @Nullable IntPredicate values);

    /**
     * Like {@link #assignmentIterator(int, IntPredicate)}, but only distinguishing assignments to the
     * variables in {@code support}, which must contain the function's own support.
     */
    ValuedIterator<BitSet> assignmentIterator(int function, @Nullable IntPredicate values, BitSet support);

    /**
     * Iterates over all root-to-leaf paths of the given {@code function} in lexicographic ascending order,
     * with {@link ValuedIterator#value()} yielding the value the path leads to. A path constrains only the
     * variables actually tested along it; every variable outside its {@link BinaryPath#support support} is a
     * "don't care".
     *
     * <p><b>Note:</b> The returned {@link BinaryPath} is the same object every time, modified in-place. If
     * all paths should be gathered into a collection, they have to be {@link BinaryPath#copy() copied} after
     * each call to {@link Iterator#next()}.</p>
     *
     * @see #forEachPath(int, PathConsumer)
     */
    ValuedIterator<BinaryPath> pathIterator(int function);
    ...
    interface ValuedIterator<E> extends Iterator<E> {
        /**
         * The value the underlying function takes on the element most recently returned by
         * {@link #next()}. Must not be called before the first {@code next()}.
         */
        int value();
    }
}
