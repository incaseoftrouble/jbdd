...
final class SyntaxTree {
    ...
    static SyntaxTree and(SyntaxTree left, SyntaxTree right) {
        return new SyntaxTree(new SyntaxTreeBinaryOperation(left.getRootNode(), right.getRootNode(), BinaryType.AND));
    }
    ...
    private static SyntaxTreeNode buildReplacementTreeRecursive(
            SyntaxTreeNode currentNode, Map<Integer, SyntaxTree> replacements) {
        ...
        throw new IllegalArgumentException(
                "Unknown type " + currentNode.getClass().getSimpleName());
    }
    ...
    static SyntaxTree equivalence(SyntaxTree left, SyntaxTree right) {
        return new SyntaxTree(
                new SyntaxTreeBinaryOperation(left.getRootNode(), right.getRootNode(), BinaryType.EQUIVALENCE));
    }

    static SyntaxTree ifThenElse(SyntaxTree first, SyntaxTree second, SyntaxTree third) {
        return new SyntaxTree(new SyntaxTreeTernaryOperation(
                first.getRootNode(), second.getRootNode(), third.getRootNode(), TernaryType.ITE));
    }

    static SyntaxTree implication(SyntaxTree left, SyntaxTree right) {
        return new SyntaxTree(
                new SyntaxTreeBinaryOperation(left.getRootNode(), right.getRootNode(), BinaryType.IMPLICATION));
    }
    ...
    static SyntaxTree or(SyntaxTree left, SyntaxTree right) {
        return new SyntaxTree(new SyntaxTreeBinaryOperation(left.getRootNode(), right.getRootNode(), BinaryType.OR));
    }

    static SyntaxTree xor(SyntaxTree left, SyntaxTree right) {
        return new SyntaxTree(new SyntaxTreeBinaryOperation(left.getRootNode(), right.getRootNode(), BinaryType.XOR));
    }
    ...
    static final class SyntaxTreeBinaryOperation extends SyntaxTreeNode {
        ...
        SyntaxTreeBinaryOperation(SyntaxTreeNode left, SyntaxTreeNode right, BinaryType type) {
            this.left = left;
            this.right = right;
            this.type = type;
    ...
    static final class SyntaxTreeTernaryOperation extends SyntaxTreeNode {
        ...
        SyntaxTreeTernaryOperation(
                SyntaxTreeNode first, SyntaxTreeNode second, SyntaxTreeNode third, TernaryType type) {
            this.first = first;
            this.second = second;
            this.third = third;
            this.type = type;
